...
-(void)willMoveToSuperview:(UIView *)newSuperview {
	...
	CGPoint offset = CGPointMake((self.bounds.size.width-90.0)/2, (self.bounds.size.height-90.0)/2);
	circleLayer.path = [UIBezierPath bezierPathWithOvalInRect:CGRectMake(offset.x, offset.y, 90.0, 90.0)].CGPath;
...
-(void)updateSlices {
    ...
    if([_pieColors count] == [_sliceValues count]){
        for (NSNumber *num in _normalizedValues) {
            ...
            slice.fillColor = [_pieColors objectAtIndex:index];
